...
data=data[data['file']!='30546757_154837607_1.txt']
...
def clean_text(note_text: str):
    ...
    #note_text = re_punc.sub('' , w) for w in note_text] 

    
    # filter out stop words
    stop_words = set(stopwords.words('english'))
    note_text = ' '.join([word for word in note_text.split() if word not in stop_words])
    #note_text = [w for w in note_text if not w in stop_words]
    ...
    #note_text = [w.lower() for w in note_text]
...
for index, row in data.iterrows():
        print(index,row.subjective_start)
        #no section
...
for index, row in df_annot_subset.iterrows():
    for index2, rowdata in data.iterrows():
        #print(str(rowdata.file).split('.txt')[0])
        if row.file==str(rowdata.file).split('.txt')[0]:
            #print(row.tag)
            #print(row.begin,row.end,rowdata.subjective_start,rowdata.subjective_end)
...
X_array_llama_13=np.load('X_array_llama_13.npy')
    ...
    for l in range(0,1,1):
        for j in range(22,23,2):
            ...
            if np.abs(y_test.sum()-y_val.sum())<10:
                ...
                clinical_test= clinical[idn_test]
                clinical_val= clinical[idn_val]
                clinical_train= clinical[idn_train]

                group_test= group[idn_test]
                group_val= group[idn_val]
                group_train= group[idn_train]

                model = Model(len(X_array[0]),len(clinical[0]))
                ...
                for i in range(epochs):
                    model.fit([X_train,clinical_train], y_train, epochs=1, batch_size=batch_size, validation_data=([X_val,clinical_val], y_val),verbose=0)

                    #model.fit(train_ds, epochs=1, batch_size=batch_size,verbose=0)
                    y_train_pre = model.predict([X_train,clinical_train])
                    #print('Train')
                    train_auc = roc_auc_score(y_train,y_train_pre)
                    #print(auc)
                    #aucx_train.append(auc)
                    ...
                    y_val_pre = model.predict([X_val,clinical_val])
                    #print('Val')
                    ...
                    y_test_pre = model.predict([X_test,clinical_test])
                    #print('Test')
                    ...
                    test_pa_auc = roc_auc_score(y_test_pa,y_test_pa_pre)
                    #print(auc)

                    #aucx_test.append(auc)
                    ...
                    #new_row=pd.DataFrame(model_type,lr,batch_size,dense_node,se,ranu,i,train_auc,val_auc,test_auc)
                    #performance = pd.concat([performance,new_row],ignore_index=True)
                    performance.append(new_row)
                    '''
                    if i==30:
                        model.save("model_llama13.h5")
                        print("Saved model to disk")
                        predictions = pd.DataFrame(columns=['index','y_pre','y'])
                        predictions['index']=np.concatenate((group1_train,group1_val,group1_test))
                        predictions['y_pre']=np.concatenate((y_train_pre,y_val_pre,y_test_pre))
                        predictions['y']=np.concatenate((y_train,y_val,y_test))
                    '''    
...
performance=pd.DataFrame(performance, columns=colnames)
performance.to_csv("performance_llama_13.csv", index=False)
#predictions.to_csv("predictions_llama_13.csv", index=False)


data= pd.read_csv('predictions_llama_13.csv')
train=data[data['train_val_test']==1]
y_train=train['y']
p_lg_train=train['y_pre']
val=data[data['train_val_test']==2]
y_val=val['y']
p_lg_val=val['y_pre']
test=data[data['train_val_test']==3]
y_test=test['y']
p_lg_test=test['y_pre']
...
y_pred_train = (p_lg_train >= 0.3).astype(bool)
y_pred_val = (p_lg_val >= 0.3).astype(bool)
y_pred_test = (p_lg_test >= 0.3).astype(bool)
...
acc_train = np.round(accuracy_score(y_pred_train, y_train), decimals=4)
...
prec_train = np.round(precision_score(y_pred_train, y_train), decimals=4)
prec_val = np.round(precision_score(y_pred_val, y_val), decimals=4)
prec_test = np.round(precision_score(y_pred_test, y_test), decimals=4)
